...
def get_google_creds() -> dict:
    ...
    Else try to fetch Google credentials from Google
    using settings.GOOGLE_CLIENT_SECRET.
...
def fetch_from_google_sheets(
    orders_amount=settings.ORDERS_DEFAULT_AMOUNT,
) -> list:
    ...
    try:
        ...
        if len(orders) == orders_amount - 1:
            return fetch_from_google_sheets(
                orders_amount + settings.ORDERS_AMOUNT_INCREASE_BY,
            )
        return orders
